...
# Standard Russian BDI text (matches app/DLL strings; item 2 uses common typo fix "будущим").
ANSWER_GROUPS = [
    [
...
beck = {
    ...
    "comment": "Прежде, чем сделать свой выбор, убедитесь, что Вы прочили Все утверждения в каждой группе",
    "analyzerId": "beck",
    "singleAnswer": False,
    "questions": [
        {
            "answers": [
                {"ball": index, "text": text}
...
out = Path(__file__).resolve().parents[1] / "PsychologyApp.Presentation/Resources/Raw/tests/beck.json"
out.write_text(json.dumps(beck, ensure_ascii=False, indent=2), encoding="utf-8")
print(f"Wrote {len(ANSWER_GROUPS)} questions -> {out}")
