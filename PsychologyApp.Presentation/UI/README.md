...
Design tokens, reusable components, and layouts used by feature pages (`Views/`).
...
5. **Press feedback on taps:** use `ButtonView` or list cards (`TechniqueListCardView`, `TestListCardView`, …) — they attach `VisualElementPressFeedback` automatically. Custom tap zones on a page get coverage via `PressFeedbackHost.AttachToPage` (wired in `MauiPageFactory`). Optional `PressFeedbackBehavior` with `AttachToAllTapTargets` for one-off layouts.
