...
public class MusicPlayerViewModel : BaseViewModel
{
    ...
    private void HideAll()
    {
        ...
        {
            Audio element = SelectedItems.ElementAt(i);

            element.AudioPlayer?.Stop();

            SelectedItems[i] = element;
        }
    }
    ...
    public void ToExecute(string file)
    {
        ...
        Audio? item = SelectedItems.FirstOrDefault(x => x.File == file);
        ...
        if (item.IsPlaying is false && item.AudioPlayer is not null)
        {
