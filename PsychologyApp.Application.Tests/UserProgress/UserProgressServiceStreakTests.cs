...
public sealed class UserProgressServiceStreakTests
{
    ...
    private sealed class FakeUserProgressRepository : IUserProgressRepository
    {
        ...
        public Task<IReadOnlyList<(string TestId, int Count)>> GetTestResultCountsAsync(IReadOnlyList<string> testIds, CancellationToken cancellationToken = default) =>
            Task.FromResult<IReadOnlyList<(string TestId, int Count)>>([]);
        ...
        public Task<string?> GetLastSessionNoteAsync(string itemKey, CancellationToken cancellationToken = default) => Task.FromResult<string?>(null);
